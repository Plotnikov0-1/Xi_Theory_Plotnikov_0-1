...
from qmt import dialog, dialog_constant, master_map, faces, whisper, three_quark, roman
...
def build():
    ...
    A(P(f"Единая константа диалога (выведена): "
        f"<b>Ω_D = (φ−1)·e^{{i·2π/φ²}}</b> — модуль |Ω_D|=φ−1={dialog_constant.modulus():.4f} "
        f"(расстояние между аспектами), фаза = золотой угол "
        f"{dialog_constant.phase_deg():.3f}° (поворот, рождающий миры), e — носитель "
        f"(входит точно). |Ω_D|<1 ⇒ спираль сходится (аспекты не важны). Точная "
        f"φπ-опора: Γ*=π/(4φ)={dialog_constant.gamma_star_exact():.4f}."))
    ...
    A(fig("29_dialog_constant.png",
          "Рис. 1б. Ω_D=(φ−1)·e^{i·2π/φ²}: из любого аспекта спираль сходится "
          "(аспекты не важны); шаг = поворот на золотой угол + сжатие φ−1 = рождение миров.",
          max_h=7.6 * cm))
    A(PageBreak())
